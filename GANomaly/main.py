...
from GANomaly.data_utils import *
from GANomaly.model import *
...
def anomaly_detection(data_type):
    ...
    with open('./results/detection_results_ganomaly_' + str(data_type) +'.txt', 'w') as f:
        ...
        for file in files:
            ...
            samples_train_data, samples_val_data, flag = read_train_data(opt.isize, file=data_name,
                                                                   step=opt.step)
            if flag == 0:
                continue
            ...
            samples_test_data, test_label = read_test_data(opt.isize, file=data_name)

            test_data = DataLoader(dataset=samples_test_data, batch_size=opt.batchsize)

            model = Ganomaly(opt, train_data, val_data, test_data, test_label, device)

            train_time, epoch_time = model.train()


            f1, pre, rec, tp, tn, fp, fn, latency, test_time = model.test(test_data, test_label)
