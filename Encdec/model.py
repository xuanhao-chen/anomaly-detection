...
class LSTMED(Base,deepBase, PyTorchUtils):
    ...
    def __init__(self, name: str='LSTM-ED', num_epochs: int=10, batch_size: int=20, lr: float=1e-3,
                 ...
                 seed: int=12345, gpu: int = 1, details=True,contamination=0.05):
    ...
    def fit(self, data):
        ...
        self.lstmed = LSTMEDModule(data.shape[1], self.hidden_size,
        ...
        for ts_batch in train_gaussian_loader:
            ...
            error_vectors += list(error.view(-1, data.shape[1]).data.cpu().numpy())
    ...
    def decision_function(self, data):
        ...
        for _, ts in enumerate(data_loader):
            ...
            score = -mvnormal.logpdf(error.view(-1, data.shape[1]).data.cpu().numpy())
        ...
        if self.details:
            ...
            lattice = np.full((self.sequence_length, data.shape[0], data.shape[1]), np.nan)
