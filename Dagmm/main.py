...
from Dagmm.dagmm import *
import numpy as np
import os
from Dagmm.eval_methods import *
...
def anomaly_detection(data_type, num_epochs, gpu, device):

    with open('./results/detection_results_' + str(data_type) +'.txt', 'w') as f:
        ...
        for file in files:
            ...
            model = DAGMM(num_epochs = num_epochs, device=device, contamination=contamination, lambda_energy=0.1, gpu=gpu,
                          lambda_cov_diag=0.005, lr=1e-3, batch_size=50, gmm_k=3, normal_percentile=80, sequence_length=30, autoencoder_args=None)
            ...
            epoch_time = train_time / num_epochs
...
device = "cpu"
print(device)
anomaly_detection(data_type, num_epochs, gpu, device)
