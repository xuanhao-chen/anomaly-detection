...
from Dagmm.algorithm_utils import deepBase, PyTorchUtils
from Dagmm.autoencoder import AutoEncoderModule
from Dagmm.lstmencdec import LSTMEDModule
from Dagmm.base import Base
...
class DAGMM(Base, deepBase, PyTorchUtils):
    ...
    def __init__(self, num_epochs=10, device = 'cpu', lambda_energy=0.1, lambda_cov_diag=0.005, lr=1e-3, batch_size=50, gmm_k=3,
                 normal_percentile=80, sequence_length=100, autoencoder_type=AutoEncoderModule, autoencoder_args=None,
                 hidden_size: int = 5, seed: int = 12345, gpu: int = 0, details=True, contamination=0.05):
    ...
    def fit(self, data):
        ...
        self.hidden_size = 5 + int(data.shape[1] / 20)
        autoencoder = self.autoencoder_type(data.shape[1], hidden_size=self.hidden_size, **self.autoencoder_args)
        self.dagmm = DAGMMModule(autoencoder, n_gmm=self.gmm_k, latent_dim=self.hidden_size + 2,
                                 seed=self.seed, gpu=self.gpu, device=self.device1)
    ...
    def decision_function(self, data):
        ...
        test_energy = np.full((self.sequence_length, data.shape[0]), np.nan)

        encodings = np.full((self.sequence_length, data.shape[0], self.hidden_size), np.nan)
        decodings = np.full((self.sequence_length, data.shape[0], data.shape[1]), np.nan)
        euc_errors = np.full((self.sequence_length, data.shape[0]), np.nan)
        csn_errors = np.full((self.sequence_length, data.shape[0]), np.nan)
...
class DAGMMModule(nn.Module, PyTorchUtils):
    ...
    def __init__(self, autoencoder, n_gmm, latent_dim, seed: int, gpu: int, device):
    ...
    def compute_energy(self, z, phi=None, mu=None, cov=None, size_average=True):
        ...
        cov_inverse = cov_inverse.to(self.device1)
